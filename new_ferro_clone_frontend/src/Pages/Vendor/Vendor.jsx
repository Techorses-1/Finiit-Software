  ...
  const handleBulkUpload = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/vendors/bulk-upload`,
        formData,
        {
          headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to upload vendors');
      }

      toast.success(`Successfully uploaded ${data.insertedCount} vendors!`);

      // Refresh vendor list
      const fetchResponse = await axios.get(`${import.meta.env.VITE_API_URL}/vendors/get-vendors`);
      const vendorsData = fetchResponse.data;
      ...
      toast.error(error.response?.data?.message || 'Error uploading vendors');
    } finally {
